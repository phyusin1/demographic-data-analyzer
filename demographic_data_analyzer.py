...
def calculate_demographic_data(print_data=True):
  ...
  lower_education_rich = round(
    non_percentage_lower / len(lower_education) * 100, 1)

  # What is the minimum number of hours a person works per week (hours-per-week feature)?
  min_work_hours = df.loc[(df["hours-per-week"] == 1)]

  # What percentage of the people who work the minimum number of hours per week have a salary of >50K?
  num_min_workers = df.loc[(df["hours-per-week"] == 1)
                           & (df["salary"] == ">50K")]

  rich_percentage = round(100 * len(num_min_workers) / len(min_work_hours), 1)
  ...
  highest_earning_country = (new_df /
                             new_df2).sort_values(ascending=False).index[0]
